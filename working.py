...
class ConcreteFingerRegionRecognizer(FingerRegionRecognizerTemplate):
    ...
    def unnormilize_image(self, img):
        return img * (self.orig_max - self.orig_min) + self.orig_min
    
    def normalize_and_segment(self, img, block_size=16, std_threshold=0.05):
        img = img[:-32, :]
        self.orig_max = np.max(img)
        self.orig_min = np.min(img)
    ...
    def count_fingerprint_ridges(self, image):
        ...
        result_image = self.draw_ridges_count_on_region(best_region, image, thin_image, block_size)
        return result_image
